...
use App\Models\InwardDc;
use App\Models\InwardDcDetail;
use Illuminate\Http\Request;
use Illuminate\Support\Facades\DB;
use Maatwebsite\Excel\Facades\Excel;
use Barryvdh\DomPDF\Facade as PDF;

class InwardDCController extends Controller
{   
    public function index()
    {
        ...
        $inwd_dc_order = DocOrder::where('fy_id', $fy->id)->first();

        $customers = Contact::orderBy('id', 'asc')->get();
        
        $invoice_no = 'LB-DC-'.$financial_year.'-'.$inwd_dc_order->inward_dc_no;
        
        return view('logics.manufacture.inward_dc.add_inwd_dc', compact('invoice_no', 'customers'));
    }

    public function inwd_dc_master()
    {
        $invoices =  DB::table('inward_dcs')->orderBy('id', 'desc')->get();
        $invoice_details =  DB::table('inward_dc_details')->get();

        $from_date=date('Y-m-d');
        $to_date=date('Y-m-d');
        
        return view('logics.manufacture.inward_dc.inwd_dc_master', compact('invoices', 'invoice_details', 'from_date', 'to_date'));
    }

    public function inwd_dc_master_filter(Request $request)
    {
        ...
        return view('logics.manufacture.inward_dc.inwd_dc_master', compact('invoices', 'from_date', 'to_date'));
    }
    ...
    public function inwd_dc_item_insert(Request $request)
    {   
        ...
        $datas = InwardDcDetail::create($input);
    ...
    public function inwd_dc_item_remove(Request $request)
    {
        $invoiceNo = $request->invoiceNo;
        $itemType = $request->itemType;
        $itemCode = $request->itemCode;
        ...
        $datas = InwardDcDetail::where('invoice_no', $invoiceNo)->where('item_code', $itemCode)->delete();
    ...
    public function inwd_dc_store(Request $request)
    {
        ...
        $input['estimated_amount'] = $estAmount;
        $input['client_note'] = $client_note;
        

        $datas = InwardDc::create($input);

        if($datas){
            $fy = FinancialYear::orderBy('id', 'desc')->first();
            $increment_inwd_dc_no = DB::table('doc_orders')->where('fy_id', $fy->id)->increment('inward_dc_no');

            if($increment_inwd_dc_no){
                return redirect()->to('inwd_dc_master')->with('success', 'Inward DC Created!');
            }else {
                return redirect()->back()->with('warning', 'Inward DC Created! Without Incrementing Inwd DC Count');
            }
        } else {
    ...
    public function inwd_dc_edit($invoice_no)
    {
        $invoices =  DB::table('inward_dcs')->where('invoice_no', $invoice_no)->first();
        $invoice_details =  DB::table('inward_dc_details')->where('invoice_no', $invoice_no)->get();
        $customers = Contact::orderBy('id', 'asc')->get();
        $customer_id = Contact::where('id', $invoices->customer_id)->pluck('id')->first();

        return view('logics.manufacture.inward_dc.inwd_dc_edit', compact('invoice_no', 'invoices', 'invoice_details','customers','customer_id'));
    }

    public function inwd_dc_update(Request $request)
    {
        $invoice_no = $request->invoice_no;
        $invoice_date = $request->invoice_date;
        $inward_bill_type = $request->invoice_type;
        ...
        $invoice = InwardDc::where('invoice_no', $invoice_no)->first();

        if ($invoice) {
            $invoice->update($input);
            return redirect()->to('inwd_dc_master')->with('success', 'Inward DC Updated!');
        } else {
            return redirect()->back()->with('error', 'Inward DC not found!');
        }
    }

    public function inwd_dc_delete($invoice_no)
    {
        $invoice_details = DB::table('inward_dc_details')->where('invoice_no', $invoice_no)->get();
        ...
        $inv_items = DB::table('inward_dc_details')->where('invoice_no', $invoice_no)->delete();
        $inv_del = DB::table('inward_dcs')->where('invoice_no', $invoice_no)->delete();
        
        $remember =   isset($inv_del) ? redirect()->back()->with('error', 'Inward DC deleted successfully!') : redirect()->back()->with('warning', 'Something went wrong please try again!');
        return $remember;
    }

    public function inwd_dc_print($invoice_no)
    {
    ...
    public function generate_inwd_dc_pdf($invoice_no)
    {
