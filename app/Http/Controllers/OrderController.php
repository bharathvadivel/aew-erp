...
class OrderController extends Controller
{
    public function order_master()
    {
        ...
        // Fetch customers who have placed at least one order
        ...
        // Fetch order summary for customers who have placed orders
        $orderSummary = DB::table('orders')
            ->join('order_details', 'orders.invoice_no', '=', 'order_details.invoice_no')
            ->select('orders.customer_id', 'order_details.item_code', DB::raw('SUM(order_details.item_qty) as total_qty'))
            ->whereIn('orders.customer_id', $customers->pluck('id')) // Filter by customers who have orders
        ...
        return view('logics.manufacture.orders.order_master', compact('p_models', 'customers', 'summaryData'));
    }
    ...
    public function manage_orders()
    {
        ...
        $invoice_details =  DB::table('order_details')->get();

        $from_date=date('Y-m-d');
        $to_date=date('Y-m-d');
        
        return view('logics.manufacture.orders.manage_orders', compact('p_models','invoices', 'invoice_details', 'from_date', 'to_date'));
    }

    public function manage_orders_filter(Request $request)
    {
        $p_models =  DB::table('p_models')->orderBy('order', 'asc')->get();
        $invoices =  DB::table('orders')->orderBy('id', 'desc')->get();
        $invoice_details =  DB::table('order_details')->get();
    ...
    public function orders_by_customer_id($id)
    {
        ...
        return view('logics.manufacture.orders.orders_by_customer', compact('id', 'invoices'));
    }
    ...
    public function add_order()
    {
        ...
        $invoice_no = 'LB-ORD-'.$financial_year.'-'.$inv_order->order_no;
        
        return view('logics.manufacture.orders.add_order', compact('invoice_no','customers','models'));
    }

    public function order_product_select(Request $request)
    {
    ...
    public function order_item_insert(Request $request)
    {   
        ...
        $datas = OrderDetail::create($input);
    ...
    public function order_item_remove(Request $request)
    {
        ...
        $datas = OrderDetail::where('invoice_no', $invoiceNo)->where('item_code', $itemCode)->delete();
    ...
    public function order_store(Request $request)
    {
        ...
        $datas = Order::create($input);

        if($datas){
            $fy = FinancialYear::orderBy('id', 'desc')->first();
            $increment_inwd_dc_no = DB::table('doc_orders')->where('fy_id', $fy->id)->increment('order_no');

            if($increment_inwd_dc_no){
                return redirect()->to('manage_orders')->with('success', 'Order Created!');
            }else {
    ...
    public function order_edit($invoice_no)
    {
        $invoices =  DB::table('orders')->where('invoice_no', $invoice_no)->first();
        $invoice_details =  DB::table('order_details')->where('invoice_no', $invoice_no)->get();
        $customers = Contact::orderBy('customer_name', 'asc')->get();
        $customer_id = Contact::where('id', $invoices->customer_id)->pluck('id')->first();
        
        $models = PModel::where('status', '=', 'Enable')->orderBy('id', 'asc')->get();
        return view('logics.manufacture.orders.order_edit', compact('invoice_no', 'invoices', 'invoice_details','customers','customer_id','models'));
    }
    ...
    public function order_update(Request $request)
    {
        ...
        $invoice = Order::where('invoice_no', $invoice_no)->first();

        if ($invoice) {
            $invoice->update($input);
            return redirect()->to('manage_orders')->with('success', 'Order Updated!');
        } else {
            return redirect()->back()->with('error', 'Order not found!');
        }
    }

    public function order_delete($invoice_no)
    {
        $invoice_details = DB::table('order_details')->where('invoice_no', $invoice_no)->get();
        $inv_items = DB::table('order_details')->where('invoice_no', $invoice_no)->delete();
        $inv_del = DB::table('orders')->where('invoice_no', $invoice_no)->delete();
        
        $remember =   isset($inv_del) ? redirect()->back()->with('error', 'Order deleted successfully!') : redirect()->back()->with('warning', 'Something went wrong please try again!');
        return $remember;
    }
