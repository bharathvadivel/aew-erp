        ...
        <div class="main-content-inner">
            ...
            <div class="col-xl-12">
                <div class="card m-b-30">
                    <div class="card-body">
                        <h4 class="header-title">Production Report
                            ...
                            <table id="dataTable" style="width: 100%;text-align:center;">
                                <thead>
                                    <tr style="text-align:center;">
                                        ...
                                        <th style="text-align:center;">Production Plan Qty</th>
                                        <th style="text-align:center;">Materials to be arranged</th>
                                        <th style="text-align:center;">Total Materials Not in Stock</th>
                                ...
                                <tbody>
                                    @foreach ($p_models as $key => $vl)
                                        @php
                                            $daily_plan_qty = DB::table('daily_plans')
                                                ->where('model_code', $vl->model_code)
                                                ->sum('qty');
                                            ...
                                            $readyToBuild = DB::table('materials as a')
                                                ->select(
                                                    ...
                                                    DB::raw("ROUND((a.total_stock_qty / b.min_assembly_qty_set) * $daily_plan_qty) as calculated_column") // Directly interpolate
                                                )
                                            ...
                                            if ($readyToBuild) {
                                                ...
                                                if($result <= 0){
                                                    $result=0;
                                                    $color = "text-align:center;font-size: 14px;color:red;cursor:pointer;border-bottom: 1px dashed red;";
                                                } else {
                                                    $color = "text-align:center;font-size: 14px;color:blue;cursor:pointer;border-bottom: 1px dashed blue;";
                                                }
                                            } else {
                                                $result=0;
                                                $color = "text-align:center;font-size: 14px;color:red;";
                                            }
                                            ...
                                            $readyToBuildIncNOTwo = DB::table('materials as a')
                                                ...
                                                ->join('material_compatible_models as b', 'a.material_code', '=', 'b.material_code')
                                                ->where('b.model_code', $vl->model_code)
                                                ->having('calculated_column', '<=', 0) // Filter only values where calculated_column <= 0
                                                ->orderBy('calculated_column', 'asc')
                                                ->get();
                                                
                                            $readyToBuildIncNOCount = $readyToBuildIncNOTwo->count();
                                        ...
                                        <tr>
                                            <td>{{ $vl->model_code }}</td>
                                            <td>{{ $vl->model_desc}}</td>
                                            <td>{{ $daily_plan_qty }}</td>
                                            <td>
                                                <a data-toggle="modal" data-target="#myModal{{ $key }}" style="{{$color}}">
                                                    <span title="Click to View Why?">Check List</span>
                                                </a>
                                                <!-- Modal -->
                                                        ...
                                                        <div class="modal-content">
                                                            <div class="modal-header">
                                                                ...
                                                                <h5 class="modal-title" id="exampleModalLabel" style="text-align:left;">
                                                                    Items Stock Requirement for Production
                                                                    <br/>
                                                                    ...
                                                                    <br/>
                                                                    <span style="font-size:14px;">Required Qty To Build <strong style="color:blue;font-size:14px;">{{ $vl->model_code }}</strong>: <span style="color:blue;font-weight:bold;">{{ $daily_plan_qty }}</span></span>
                                                                    <br/>
                                                                ...
                                                                <table class="table table-striped table-bordered" style="width:100%;">
                                                                    <thead class="text-capitalize" style="position:sticky;top:0;z-index: 1;">
                                                                        <tr>
                                                                            ...
                                                                            <th style="text-align:center;background:#e0e0e0;border:2px solid #ABB2B9!important;">Description</th>
                                                                            <th style="text-align:center;background:#e0e0e0;border:2px solid #ABB2B9!important;">Required Stock</th>
                                                                            <th style="text-align:center;background:#e0e0e0;border:2px solid #ABB2B9!important;">Available Stock</th>
                                                                    ...
                                                                    <tbody style="position:relative;padding-top: 40px;">
                                                                        @php
                                                                            $materialstocks = DB::table('materials as a')
                                                                                ...
                                                                                ->get();
                                                                        @endphp
                                                                        @foreach ($materialstocks as $key => $vols)
                                                                            @php
                                                                            ...
                                                                            @endphp
                                                                            <tr style="background: {{ $key == 0 ? '#D1FFBD' : '' }};">
                                                                                <td>{{ $key + 1 }}</td>
                                                                                <td>{{ $vols->material_code }}</td>
                                                                                <td>{{ $vols->material_desc }}</td>
                                                                                <td>{{ $vols->min_assembly_qty_set * $daily_plan_qty }}</td>
                                            ...
                                            <td style="text-align:center;">
                                                <a data-toggle="modal" data-target="#myNoModal{{ $key }}" style="{{$colorNo}}">
                                                    <span title="Click to View How?">{{ $readyToBuildIncNOCount }}</span>
                                                </a>
                                                <!-- Modal -->
