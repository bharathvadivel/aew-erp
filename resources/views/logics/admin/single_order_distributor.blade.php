        ...
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    ...
                    $locations = DB::table('userlocations')->where('partner_id',$to_id)->get();



                    @endphp
                    <form method="post" action="{{route('distributorinvoice.order')}}">
                        @csrf
                        <div class="mb-3">
                        ...
                        <input type="hidden" name="from_location_id" class="location_id">
                        <input type="hidden" value="{{$let->partner_id}}" name="partner_id">
                        <input type="hidden" value="{{$let->partner_type}}" name="partner_type">
                        <input type="hidden" value="{{$let->name}}" name="partner_name">
                        <input type="hidden" value="{{$let->phone}}" name="phone">
                        <input type="hidden" value="{{$get_dis->credit_limit}}" name="credit_limit">
                        <input type="hidden" value="{{$get_dis->available_limit}}" name="available_limit">
                        <input type="hidden" value="{{$get_dis->credit_days}}" name="credit_days">
                        <input type="hidden" value="{{$let->address}}" name="address">
                        <input type="hidden" value="{{$where->pincode}}" name="pincode">
                        <input type="hidden" value="{{$let->city}}" name="city">
                        <input type="hidden" value="{{$let->district}}" name="district">
                        <input type="hidden" value="{{$let->location_id}}" name="sub_location_id">
                        <input type="hidden" value="{{$let->address}}" name="delivery_address">
                        <input type="hidden" value="{{$let->location_id}}" name="delivery_location_id">
                        <input type="hidden" name="state" value="{{$let->state}}">
                        <input type="hidden" name="country" value="{{$let->country}}">
                        ...
                        @foreach ($order as $key=>$vl)
                        @php
                        $ch_qty= DB::table('apiorderlists')->where('order_id',$order_id)->where('description',$vl->description)->sum('qty');
                        $product= DB::table('products')->where('id',$vl->product_id)->first();
                        $data = DB::table('disserials')->where('model_no',$vl->model_no)->where('description',$vl->description)->where('status','unused')->get();
                        @endphp
                        <input type="hidden" name="gategory[]" value="{{$vl->category_name}}">
                        <input type="hidden" name="description[]" value="{{$vl->description}}">
