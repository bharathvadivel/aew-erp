...
<body>
    ...
    *====================================-->
                    ...
                    <div class="card">
                        <div class="card-body">
                            <h4 class="header-title" style="display:flex;justify-content: space-between;align-content: space-around;">
                                Returned Spare Parts List

                                <a href="{{ route('scp.create.return.order') }}" class="btn btn-primary btns"> <i class="fa fa-plus-circle"></i> Create Return Order</a>
                            </h4>
                            <br>
                                ...
                                <table id="dataTable2" class="display" style="width:100%">
                                    ...
                                    <tbody>
                                        ...
                                        @foreach ($row as $key => $vl)
                                            @php
                                                $count = DB::table('scp_serials')->where('scp_return_no', $vl->scp_invoice_no)->count();
                                                if($vl->supplied_to_type == 'Service Center')
                                                {
                                                    $to = DB::table('services')->where('service_id', $vl->supplied_to)->first();
                                                    $name=$to->service_center_name;
                                                }
                                                elseif($vl->supplied_to_type == 'Warehouse')
                                                {
                                                    $to = DB::table('warehouses')->where('warehouse_id', $vl->supplied_to)->first();
                                                    $name=$to->name;
                                                }
                                            ...
                                            <tr>
                                                ...
                                                <td>
                                                    ...
                                                    <a target="_blank" href="{{url('scp_return_order_pdf/'.$vl->scp_invoice_no)}}"><i  data-placement="top" title="Invoice" class="fa fa-eye" style="color:green"></i></a>
    ...
    @foreach ($row as $key=>$vl)
    @php
    $count = DB::table('scp_serials')
    ->where('scp_return_no', $vl->scp_invoice_no)
    ->get();
    @endphp
